...
describe('Registration', () => {
    it('Deve realizar o cadastro do Usuario', () => {
        ...
        cy.wait('@postRegister').its('response.statusCode').should('equal', 200)
    });
...
describe('New Bet', () => {
    ...
    it('Criar um jogo Lotofácil aleatório', () => {
        cy.get('[data-cy="id1"]').click();
        cy.get('[data-cy="btnComplete"]').click();
        cy.get('[data-cy="btnAdd"]').click();

        cy.get('[data-cy="cardTypeGame"]').should('have.text','Lotofácil');
    });
    it('Criar um jogo Mega-Sena aleatório', () => {
        cy.get('[data-cy="id2"]').click();
        cy.get('[data-cy="btnComplete"]').click();
        cy.get('[data-cy="btnAdd"]').click();

        cy.get('[data-cy="cardTypeGame"]').should('have.text','Mega-Sena');
    });
    it('Criar um jogo Quina aleatório', () => {
        cy.get('[data-cy="id3"]').click();
        cy.get('[data-cy="btnComplete"]').click();
        cy.get('[data-cy="btnAdd"]').click();

        cy.get('[data-cy="cardTypeGame"]').should('have.text','Quina');
    });
    it('Realizar compra com sucesso', () => {
        ...
        cy.wait('@postNewBet').its('response.statusCode').should('equal', 200)
    });
    it('Erro ao realizar compra: Valor da compra menor que 5 reais', () => {
        ...
        cy.wait('@postNewBet').its('response.statusCode').should('equal', 400);
        cy.get('.Toastify__toast--warning').should('have.text','Valor mínimo autorizado: R$5,00.');

    });
    it('Erro ao adicionar no carrinho: Número selecionado menor que o desejado', () => {
        ...
        cy.get('[data-cy="btnAdd"]').click();
        cy.get('.Toastify__toast--warning').should('have.text','Selecione 5 números. Faltam 2 números para completar!');
    });
});
